...
Both `id` and `tempId` are optional on `Box`. If a box has neither (e.g., from a malformed API response or a future code path), this silently returns `undefined` typed as `string`, which cascades into broken Map lookups, wrong history deltas, and rendering glitches.

**Prompt:**

> In `boxes.interface.ts`, add a runtime guard to `getBoxId()`:
>
> ```typescript
...
> ```
>
> This converts a silent type lie into an explicit error that surfaces immediately during development. Also consider making the `Box` type enforce at least one of them via a discriminated union in a follow-up (see issue #16).

---

### 9. Non-null assertion `state.bgCanvas()!` used inside pointer-event handler

**File:** `src/components/canvas-viewpoint/handlers/pointer-event-handler.ts` — `handleMagicDetection`
...
`handleMagicDetection` is only called when `state.bgCanvas()` is truthy (checked in the caller), but this pattern is fragile — the assertion is context-dependent and not locally safe.

**Prompt:**

> In `pointer-event-handler.ts`, in the `handleMagicDetection` private static method, replace `const bgCanvas = state.bgCanvas()!` with a proper null check:
>
> ```typescript
> const bgCanvas = state.bgCanvas();
> if (!bgCanvas) return;
